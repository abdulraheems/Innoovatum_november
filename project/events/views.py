...
from .forms import CommentForm,EventPostForm
...
def event_detail(request, event):
    event=get_object_or_404(Event,event_slug=event,status='published')
...
def add_events(request):
    if request.method=="POST":
        form = EventPostForm(data=request.POST, files=request.FILES)
        if form.is_valid():
            eventpost = form.save(commit=False)
            eventpost.event_author = request.user
            eventpost.save()
            obj = form.instance
            alert = True
            return render(request, "events/add_events.html",{'obj':obj, 'alert':alert})
    else:
        form=EventPostForm()
